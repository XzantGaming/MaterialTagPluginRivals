...
void FMaterialSlotTagEntryCustomization::AddTagToSlot(const FString& TagName)
{
	...
	// Try to get existing tag first, but allow adding even if not registered
	// (the tag name will be stored and can be resolved later)
	FGameplayTag Tag = FGameplayTag::RequestGameplayTag(FName(*TagName), false);
	if (!Tag.IsValid())
	{
		// Tag doesn't exist in GameplayTags - warn but still allow adding the raw name
		UE_LOG(LogTemp, Warning, TEXT("MaterialSlotTagEntry: Tag '%s' not found in GameplayTags. Adding as raw tag name."), *TagName);
	}
